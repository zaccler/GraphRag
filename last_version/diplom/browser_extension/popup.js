...
async function saveQuestionDraft() {
  if (!$("question")) return;
  questionDrafts[currentDraftKey()] = $("question").value;
  await chrome.storage.local.set({ questionDrafts });
}

async function clearQuestionDraft() {
  delete questionDrafts[currentDraftKey()];
  await chrome.storage.local.set({ questionDrafts });
}

function restoreQuestionDraft() {
  if (!$("question")) return;
  $("question").value = questionDrafts[currentDraftKey()] || "";
}
...
function renderLlmMode() {
  const local = llmMode === "local";
  $("llmApiBtn")?.classList.toggle("active", !local);
  $("llmLocalBtn")?.classList.toggle("active", local);
}

async function setLlmMode(mode) {
  llmMode = mode === "local" ? "local" : "api";
  await chrome.storage.local.set({ llmMode });
  renderLlmMode();
  writeOutput(`LLM mode: ${llmMode}`);
}
...
async function loadSettings() {
  const localData = await chrome.storage.local.get(["apiUrl", "role", "chatLogs", "chatLogsByUser", "questionDrafts", "llmMode"]);
  const sessionData = await chrome.storage.session.get(["role", "userEmail", "authDraft"]);
  apiUrl = localData.apiUrl || DEFAULT_API;
  llmMode = "api";
  if (localData.llmMode !== "api") await chrome.storage.local.set({ llmMode });
  ...
  questionDrafts =
    localData.questionDrafts && typeof localData.questionDrafts === "object" ? localData.questionDrafts : {};
...
async function runAskDebugFromModal() {
  ...
  writeOutput({
    action: "/ask_debug",
    status: "sending",
    request: { question, llm_mode: llmMode },
  ...
  try {
    const data = await api("/ask_debug", {
      method: "POST",
      body: JSON.stringify({ question, llm_mode: llmMode }),
    });
    closeAskDebugModal();
    writeOutput(data);
    refreshStatusBadge();
...
async function ask(debug = false, button = $("askBtn")) {
  ...
  writeOutput({ action: debug ? "/ask_debug" : "/ask", status: "sending", request: { question, llm_mode: llmMode } });
...
function bindEvents() {
  ...
  $("llmApiBtn").onclick = () => setLlmMode("api");
  $("llmLocalBtn").onclick = () => setLlmMode("local");
