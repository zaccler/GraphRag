...
def _text_files(output_dir: str):
    return sorted(Path(output_dir).glob("*.txt"))


def _has_existing_text(output_dir: str) -> bool:
    return bool(_text_files(output_dir))


def _copy_changed_text_files(source_dir: str, target_dir: str):
    changed = []
    unchanged = []
    target_root = Path(target_dir)
    target_root.mkdir(parents=True, exist_ok=True)

    for source_path in _text_files(source_dir):
...
def _parse_source_item(item: Dict, source_type: str, output_dir: str):
...
def parse_registry_sources(
    ...
    for item in sources:
        if limit is not None and len(crawled) >= limit:
            break
        ...
        has_existing = _has_existing_text(output_dir)
        refreshable = source_type in REFRESHABLE_SOURCE_TYPES

        if not force and has_existing and not refreshable:
            skipped.append({"code": code, "type": source_type, "reason": "already exists"})
            continue

        attempted += 1
        try:
            if not force and has_existing and refreshable:
                with tempfile.TemporaryDirectory(prefix=f"{code}_refresh_") as temp_dir:
                    _parse_source_item(item, source_type, temp_dir)
                    changed, unchanged = _copy_changed_text_files(temp_dir, output_dir)
                ...
                saved_files.extend(changed)
                crawled.append(
                    {
                        "root_url": item.get("url") or item.get("csv_url") or changed[0],
                        "pages_saved": len(changed),
                        "visited": 1,
                        "files": changed,
                        "updated": True,
                    }
            ...
            files, source_results = _parse_source_item(item, source_type, output_dir)
            saved_files.extend(files)
            crawled.extend(source_results)
